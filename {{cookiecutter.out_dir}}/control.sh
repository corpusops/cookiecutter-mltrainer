...
BUILD_CONTAINERS="$APP_CONTAINER"
EDITOR=${EDITOR:-vim}
...
DOCKER_USERID=${DOCKER_USERID:-$(id -u)}
BUILD_ONLY=${BUILD_ONLY-}
if [[ -n ${BUILD_ONLY} ]];then NO_BUILD=;fi
NO_BUILD=${NO_BUILD-1}
if [[ -z $NO_BUILD ]];then
    do_build
    ret=$?
    if [[ -n ${BUILD_ONLY-} ]];then exit $ret;fi
fi
export DOCKER_UID=${DOCKER_UID:-$(id -u)}
MLFLOW_FORWARD_ENV="
SHELL_DEBUG WORK_DIR PATH SKIP_SKIPGRAM SKIP_MODEL STAGE MODEL_STAGE SKIPGRAM_STAGE
DOCKER_USERID DOCKER_HOST DOCKER_IMAGE DOCKER_IMAGE_VERSION
MLFLOW_TRACKING_URI MLFLOW_TRACKING_USERNAME MLFLOW_TRACKING_PASSWORD
MLFLOW_S3_ENDPOINT_URL AWS_SECRET_ACCESS_KEY AWS_ACCESS_KEY_ID
MLFLOW_DRY_RUN"
...
#       APP_USER=mltrainer ./control.sh usershell ls /
...
#       APP_USER=mltrainer ./control.sh userexec ls /
...
#       APP_USER=mltrainer ./control.sh duserexec -> run interactive shell inside default CONTAINER
#       APP_USER=mltrainer ./control.sh duserexec foo123 -> run interactive shell inside foo123 CONTAINER
#       APP_USER=mltrainer ./control.sh duserexec mltrainer_123 ls / -> run comand inside foo123 CONTAINER
do_duserexec() {
...
#  train: launch app container in foreground (using entrypoint)
do_train() {
    stop_containers
    vv $DC run --rm --no-deps --use-aliases --service-ports -e IMAGE_MODE=train $APP_CONTAINER $@
}
...
do_test() {
    ...
    mltrainer bash -e <<'EOF'
if [ -e ../.tox ];then chown mltrainer ../.tox;fi
gosu mltrainer bash -ec '
TOX_ARGS=${TOX_ARGS-}
...
#  dcompose_mlflow: manage mlflow registry
do_dcompose_mlflow() {
    cd "$W/local/docker-mlflow/rootfs"
    MLFLOW_DC="${MLFLOW_DC:-"docker-compose"}"
    log "in $PWD"
    if [ ! -e .env ];then
        dn=$(basename $W|sed -e "s/[-_]//g")
        echo "COMPOSE_PROJECT_NAME=$dn-mlflow">.env
        ${EDITOR:-editor} .env
    fi
    $MLFLOW_DC "$@"
}

#  down_registry: start mlflow registry
do_down_registry() {
    do_dcompose_mlflow down $@
}

#  up_registry: stop mlflow registry
do_up_registry() {
    do_dcompose_mlflow up -d --force-recreate $@
}
...
do_main() {
    ...
    actions="$actions|init|up|train|pull|build|buildimages|down|rm|run"
    actions_{{cookiecutter.app_type}}="tests|test|coverage|linting|python"
    actions_mlflow="dcompose_mlflow|down_registry|up_registry"
    actions="@($actions|$actions_mlflow|$actions_{{cookiecutter.app_type}})"
    action=${1-}
    source_envs
    if [[ -n $@ ]];then shift;fi
    set_dc
