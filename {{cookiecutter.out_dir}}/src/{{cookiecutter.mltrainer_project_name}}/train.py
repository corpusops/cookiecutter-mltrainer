...
from myapp import mlflow_utils

DATA_PATH = mlflow_utils.DATA_PATH
LOGGER = logging.getLogger(__name__)
LOCAL_PREV_MODEL_PATH = DATA_PATH / "brand_matching_prev"
LOGGER = logging.getLogger(__name__)
...
parser.add_argument(
    "--train_steps", default=3, type=int, help="number of training steps"
)
mlflow_utils.config_cli(parser)


def main(argv):
    status = mlflow_utils.Status.FAILED
    version = None
    with mlflow.start_run():
        args = parser.parse_args(argv[1:])
        status_file = args.output_status
        if not status_file:
            status_file = mlflow_utils.output_status
        mlflow_utils.rm(status_file)
        ...
        #         'path': LOCAL_PREV_MODEL_PATH,
        #     },
        # }
        # mlflow_utils.download_and_load_models(mlflow_models)
        ...
        try:
            ...
            status, version = mlflow_utils.register_model(
                mlflow, args.model_name, model_path=model_path
            )
        except Exception:
            ...
            mlflow.end_run()
            # output a status needed in the pipeline to forward up to the django app
            mlflow_utils.write(status_file, status, args.model_name, version)
        return status
