...
export MLTRAINER_WSGI=${MLTRAINER_WSGI:-project.wsgi}
export MLTRAINER_LISTEN=${MLTRAINER_LISTEN:-"0.0.0.0:8000"}

# Celery variables
# export CELERY_SCHEDULER=${CELERY_SCHEDULER:-celery.beat.PersistentScheduler}
export CELERY_SCHEDULER=${CELERY_SCHEDULER:-mltrainer_celery_beat.schedulers:DatabaseScheduler}
export CELERY_LOGLEVEL=${CELERY_LOGLEVEL:-info}
export MLTRAINER_CELERY=${MLTRAINER_CELERY:-project.celery:app}
export MLTRAINER_CELERY_BROKER="${MLTRAINER_CELERY_BROKER:-amqp}"
export MLTRAINER_CELERY_HOST="${MLTRAINER_CELERY_HOST:-celery-broker}"
export MLTRAINER_CELERY_VHOST="${MLTRAINER_CELERY_VHOST:-}"
if ( echo "$MLTRAINER_CELERY_BROKER" | egrep -q "rabbitmq|amqp" );then
    burl="amqp://${RABBITMQ_DEFAULT_USER}:${RABBITMQ_DEFAULT_PASS}@$MLTRAINER_CELERY_HOST/$MLTRAINER_CELERY_VHOST/"
elif [[ "$MLTRAINER_CELERY_BROKER" = "redis" ]];then
    burl="redis://$MLTRAINER_CELERY_HOST/"
fi
export MLTRAINER__CELERY_BROKER_URL="${MLTRAINER__CELERY_BROKER_URL:-$burl}"
...
configure() {
    ...
    for i in $(find etc -name "*.envsubst" -type f 2>/dev/null);do
        di="/$(dirname $i)" \
            ...
            && cp "$i" "/$i" \
            && CONF_PREFIX="$MLTRAINER_CONF_PREFIX" confenvsubst.sh "/$i" \
            && rm -f "/$i"
    done
...
do_fg() {
    ( cd $PROJECT_DIR \
        && exec gosu $APP_USER ./manage.py runserver $MLTRAINER_LISTEN )
}
